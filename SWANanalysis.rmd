...
data <- read_tsv('/Users/aj/Downloads/32961-0001-Data.tsv', show_col_types = FALSE)
```
```{r}
head(data)
```
...
```{r}
df <- data.frame(data$RACE, data$STATUS10, data$AGE10, data$LANGINT10, data$INSURAN10, data$NOTAFFR10, data$INCOME10, data$HOTFLAS10, #predictors
...
# Filter rows where values are not negative
df <- df %>%
  filter(data.INCOME10 > 0)
```
...
```{r}
#write file
write.csv(df, "/Users/aj/Downloads/yale-main/SWAN_prelim72.csv")
dim(df)
```